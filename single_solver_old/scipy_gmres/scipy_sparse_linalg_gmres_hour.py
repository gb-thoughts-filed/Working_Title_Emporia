...
def sparse_linalg_gmres(Ldd, rhs, timer):
    counter = 0
    title = datetime.today()
    time_file = open(f"scipy_sparse_linalg_gmres{title: %B%d%Y}.txt", "a")
    start_time = datetime.now()
    t1 = time.time()

    while time.time() - t1 < timer:
        udd = sp.sparse.linalg.gmres(Ldd, rhs, tol=10**-7)
        counter += 1
...
sparse_linalg_gmres(Ldd, rhs, 3600)
